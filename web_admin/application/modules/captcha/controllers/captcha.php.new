...
class Captcha extends MX_Controller
{
    ...
    function getCaptcha($rand){
	$this->phpcaptcha('#162453','#fff',120,40,10,25);	
    }
    
    public function phptext($text,$textColor,$backgroundColor='',$fontSize,$imgWidth,$imgHeight,$dir,$fileName)
    {
        /* settings */
        $font = FCPATH.'assets/fonts/calibri.ttf';
    ...
    {	
        /* Settings */
        $text=$this->random();
        $font = FCPATH.'assets/fonts/monofont.ttf';/* font */
        $textColor=$this->hexToRGB($textColor);	
        ...
        $this->session->set_userdata('captcha_code', $text);
